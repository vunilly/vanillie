...
public class PvpListener implements Listener {
    ...
    public void onPlayerPvp(EntityDamageByEntityEvent event) {
        ...
        if (theirScale.getBaseValue() <= 0.8) {
            attacker.sendMessage(Lang.get("msg.pvp.tooLow.them", Placeholder.parsed("player", victim.getName())).getFirst());
            event.setCancelled(true);
            return;
        }

        if (PvpManager.isPvpOff(attacker.getUniqueId())) {
            attacker.sendActionBar(Lang.get("msg.pvp.cancelled.you").getFirst());
            event.setCancelled(true);
        } else if (PvpManager.isPvpOff(victim.getUniqueId())) {
            attacker.sendActionBar(Lang.get("msg.pvp.cancelled.them", Placeholder.parsed("player", victim.getName())).getFirst());
            event.setCancelled(true);
        }
